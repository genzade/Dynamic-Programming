const gridTraveller = require("../../src/memoization/grid-traveller");

describe("GridTraveller", () => {
	it("returns correct number of steps given m, n are 0, 1, repsectively", () => {
		const m = 0;
		const n = 1;
		expect(gridTraveller.gridTraveller(m, n)).toBe(0);
	});
	...
	it("returns correct number of steps given m, n are 1, 1, repsectively", () => {
		const m = 1;
		const n = 1;
		expect(gridTraveller.gridTraveller(m, n)).toBe(1);
	});
